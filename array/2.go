...
func main() {
	l1 := common.NewList([]int{8, 9, 9}) // 5	// 9 8
	l2 := common.NewList([]int{2})       // 5	// 1
...
func addTwoNumbers(l1 *common.ListNode, l2 *common.ListNode) *common.ListNode {
	...
	return common.NewList(nums)
}

func traverse(cur *common.ListNode, carryBit bool) (remainNums []int) {
	if cur == nil {
		return
	}
	for cur != nil {
		if carryBit {
			res := cur.Val + 1
			if res >= 10 {
				remainNums = append(remainNums, res%10)
				carryBit = true
			} else {
